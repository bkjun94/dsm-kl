...
#include "../codecs/max9860.h"  // feelink
...
static int fsl_asoc_card_startup(struct snd_pcm_substream *substream)
{
    ...
    // feelink
	if (priv->card_type == CARD_MAX9860
	    && !priv->is_codec_master) {
		support_rates[0] = 8000;
		support_rates[1] = 16000;
		support_rates[2] = 32000;
		support_rates[3] = 44100;
		support_rates[4] = 48000;
		constraint_rates.list = support_rates;
...
static int fsl_asoc_card_probe(struct platform_device *pdev)
{
	...
	if (of_device_is_compatible(np, "fsl,imx-audio-cs42888")) {
		...
		priv->card_type = CARD_WM8958;
	} else if (of_device_is_compatible(np, "fsl,imx-audio-max9860")) {  // feelink
		codec_dai_name = "max9860-hifi";
		priv->dai_fmt |= SND_SOC_DAIFMT_CBP_CFP;
		priv->card_type = CARD_MAX9860;
	} else {
...
static const struct of_device_id fsl_asoc_card_dt_ids[] = {
	...
	{ .compatible = "fsl,imx-audio-max9860", }, // feelink
	{}
};
